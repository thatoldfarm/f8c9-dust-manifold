...
# =============================================================================
NODE_QTY = 32          
# Changed to 2 for Absolute Stability. 1 Byte per hop = No alignment errors.
HEX_CHUNK_SIZE = 2     
...
class PolarizedEngine:
    ...
    def find_best_coordinate(self, chunk):
        best_idx = float('inf')
        best_state = (0, 0, 0, 0) 
        chunk_clean = chunk if len(chunk)%2==0 else '0'+chunk
        try:
            chunk_bytes = bytes.fromhex(chunk_clean)
        except ValueError:
            return 0, (0, 0, 0, 0)
        for const_id in [0, 1]:
            lattice = self.pi_lattice if const_id == 0 else self.e_lattice
            for pol in [0, 1]:
                for mir in [0, 1]:
                    test_chunk = chunk
                    ...
                    for f_val in range(1, 256):
                        try:
                            b_chunk = bytes.fromhex(test_chunk if len(test_chunk)%2==0 else '0'+test_chunk)
                            refracted = bytes([b ^ f_val for b in b_chunk]).hex()
        ...
        if best_idx == float('inf'):
            return int(chunk, 16), (2, 0, 0, 0)
        return int(best_idx), best_state
...
class SovereignCartographer:
    ...
    def encode(self):
        ...
        # Force ZSTD if available, otherwise GZIP
        ...
        coordinates = []
        filters = []
        print(f"    > Mapping {len(chunks)} chunks to Polarized Lattice...")
        for c in chunks:
        ...
        print(f"[SUCCESS] Polarized Crystal forged: {self.ledger_file}")
        return True

# =============================================================================
# 7. THE SOVEREIGN SPIGOT
# =============================================================================
class SovereignSpigotDecoder:
    ...
    def decode(self):
        print(f"\n[4] SPIGOT: Reifying Polarized Crystal '{self.ledger_file}'...")
...
if __name__ == "__main__":
    print("=====================================================")
    print("   f8c9 POLARIZED SOVEREIGN CRYSTAL PIPELINE")
    print("=====================================================")
    ...
    for filename in artifacts:
        ...
        cartographer = SovereignCartographer(target_path, NODE_QTY, HEX_CHUNK_SIZE)
        if cartographer.encode():
            decoder = SovereignSpigotDecoder(cartographer.ledger_file, NODE_QTY)
            decoder.decode()
