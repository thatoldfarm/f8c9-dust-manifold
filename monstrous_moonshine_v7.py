...
class ElegantCartographer:
    def __init__(self, target_file, node_qty, hex_chunk_size):
        ...
        # Required Pi digits based on chunk size to ensure all combinations are found
        self.pi_depth_map = {
    ...
    def encode(self):
        ...
        # ---------------------------------------------------------
        # ADAPTIVE ROUTER: Fallback if precomputed file is missing
        # ---------------------------------------------------------
        ...
        for chunk in chunks:
            ...
            if idx == -1:
                print(f"    [!] Critical: Chunk '{chunk}' not found in Pi. (Generate a larger Pi file)")
                return False
            indices.append(idx)
...
class ElegantSpigotDecoder:
    ...
    def decode(self, original_compressed_hex=None):
        ...
        print(f"[6] SPIGOT ENGINE: Reclaiming Hypersphere Volume via {hex_chunk_size * 4}-Bit BBP Formula...")
        restored_hex = ""
        ...
        if original_compressed_hex:
            ...
            for i, (a, b) in enumerate(zip(restored_hex, original_compressed_hex)):
                if a != b:
                    raise ValueError(f"Mismatch at position {i}")
...
if __name__ == "__main__":
    print("=====================================================")
    print("   f8c9 ADAPTIVE SOVEREIGN PIPELINE (V15.54)")
    print("=====================================================")
    ...
    for filename in artifacts:
        ...
        # We pass our ideal HEX_CHUNK_SIZE. If pi_hex_30m.txt is missing, 
        # the Cartographer will automatically fallback to 3 hex-digit resolution (0.1s run).
    ...
    print("\n=====================================================")
    print(f"[PIPELINE COMPLETE] Processed {len(artifacts)} artifacts with Adaptive Pipeline (V15.54).")
    print("=====================================================")
